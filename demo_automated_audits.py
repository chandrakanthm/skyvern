...
class MockPage:
    ...
    async def screenshot(self, full_page=False):
        return b"MOCK_SCREENSHOT_DATA_FOR_DEMO"
...
async def demo_automated_audits():
    ...
    try:
        ...
        print(f"\n   🎨 Sample Color Rules:")
        for i, color in enumerate(guidelines.colors[:3]):
            print(f"      • {color.name}: {color.hex_value} ({color.usage})")
        
        print(f"\n   📝 Sample Font Rules:")
        for i, font in enumerate(guidelines.fonts[:2]):
        ...
        mock_visual_segments = [
            {
                ...
                'visual_properties': {
                    'colors': {
                        'color': '#ff0000',  # Violation: not in brand palette
                        'background-color': '#ffffff'
                    },
                    'typography': {
                        'font-family': 'Comic Sans MS',  # Violation: not approved
                        'font-size': '24px',
                        'font-weight': 'bold'
                    },
                    'spacing': {
                        'margin': '13px',  # Violation: not standard
                        'padding': '10px'
                    }
                }
            },
            {
                ...
                'visual_properties': {
                    'colors': {
                        'color': '#333333',  # This one is compliant
                        'background-color': '#f8f9fa'
                    },
                    'typography': {
                        'font-family': 'Arial, sans-serif',  # Compliant
                        'font-size': '16px',
                        'font-weight': 'normal'
                    },
                    'spacing': {
                        'margin': '16px',  # Compliant
                        'padding': '12px'
                    }
                }
            },
            {
                ...
                'visual_properties': {
                    'colors': {
                        'color': '#ffffff',
                        'background-color': '#purple'  # Violation: not in palette
                    },
                    'typography': {
                        'font-family': 'Helvetica, sans-serif',  # Compliant
                        'font-size': '14px',
                        'font-weight': '600'
                    },
                    'spacing': {
    ...
    try:
        ...
        audit_result = await compliance_engine.audit_visual_segments(
            mock_visual_segments, 
            "https://demo-website.example.com"
        )
    ...
    try:
        mock_page = MockPage("https://demo-website.example.com")
        
        print("🎨 Creating annotated screenshot with violation markers...")
        
        screenshot_result = await annotation_engine.generate_annotated_screenshot(
            mock_page,
            audit_result,
